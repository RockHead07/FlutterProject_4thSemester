...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Row(
        children: [
          SidebarWidget(selectedIndex: _selectedIndex, onItemSelected: (i) => setState(() => _selectedIndex = i)),
          Expanded(child: Column(children: [_buildTopBar(), Expanded(child: _buildPageContent())])),
        ],
      ),
    );
  }
  ...
  Widget _buildPageContent() {
    switch (_selectedIndex) {
      ...
      case 1: return UsersPage(token: widget.user.token);
      default: return _buildPlaceholder(_pageTitles[_selectedIndex]);
    }
  }
  ...
  Widget _buildDashboardOverview() {
    return BlocBuilder<UsersBloc, UsersState>(builder: (context, state) {
      ...
      return SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildWelcomeBanner(), const SizedBox(height: 16),
        LayoutBuilder(builder: (context, constraints) {
          final isWide = constraints.maxWidth > 600;
          final cardWidth = isWide ? (constraints.maxWidth - 48) / 4 : (constraints.maxWidth - 16) / 2;
          return Wrap(spacing: isWide ? 16 : 10, runSpacing: isWide ? 16 : 10, children: [
            SizedBox(width: cardWidth.clamp(140.0, 400.0), child: StatCardWidget(title: 'Total Users', value: totalUsers.toString(), subtitle: 'Semua akun', changePercent: 12, isPositive: true)),
            SizedBox(width: cardWidth.clamp(140.0, 400.0), child: StatCardWidget(title: 'Active', value: activeUsers.toString(), subtitle: 'Akun aktif', changePercent: 8, isPositive: true)),
            SizedBox(width: cardWidth.clamp(140.0, 400.0), child: StatCardWidget(title: 'Admins', value: adminCount.toString(), subtitle: 'Role admin', changePercent: 0, isPositive: true)),
            SizedBox(width: cardWidth.clamp(140.0, 400.0), child: StatCardWidget(title: 'Inactive', value: inactiveUsers.toString(), subtitle: 'Nonaktif', changePercent: inactiveUsers > 0 ? 5 : 0, isPositive: false)),
          ]);
        }),
        const SizedBox(height: 16),
        _buildChartCard(title: 'Distribusi Status User', subtitle: 'Aktif vs nonaktif', child: Padding(padding: const EdgeInsets.all(16), child: DonutChart(activeRate: activeRate, activeCount: activeUsers, inactiveCount: inactiveUsers, total: totalUsers))),
        const SizedBox(height: 16),
        _buildChartCard(title: 'Registrasi Bulanan', subtitle: 'User baru per bulan', child: const SizedBox(height: 160, child: Padding(padding: EdgeInsets.fromLTRB(12, 8, 12, 0), child: VerticalBarChart(data: [4, 7, 5, 9, 6, 12, 8], labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'])))),
        const SizedBox(height: 16),
