...
class _UsersPageState extends State<UsersPage> {
  ...
  List<UserEntity> _filterUsers(List<UserEntity> users) {
    return users.where((user) {
      ...
      final matchesSearch = q.isEmpty || user.username.toLowerCase().contains(q) || user.email.toLowerCase().contains(q) || (user.firstName ?? '').toLowerCase().contains(q) || (user.lastName ?? '').toLowerCase().contains(q);
  ...
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _buildHeader(), const SizedBox(height: 14), _buildSearchAndFilter(), const SizedBox(height: 14), Expanded(child: _buildUsersContent()),
    ]));
  }
  ...
  Widget _buildHeader() {
    return Row(children: [
      ...
      SizedBox(height: 38, child: ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), elevation: 0),
      )),
    ]);
  }
  ...
  Widget _buildUsersContent() {
    return BlocBuilder<UsersBloc, UsersState>(builder: (context, state) {
      if (state.status == UsersStatus.loading && state.users.isEmpty) return const Center(child: CircularProgressIndicator(color: AppColors.primary));
      if (state.status == UsersStatus.failure && state.users.isEmpty) return _buildErrorState(state.errorMessage ?? 'Terjadi kesalahan');
  ...
  Widget _badge(String text, Color textColor, Color bgColor, {Color? dotColor}) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(20)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        if (dotColor != null) ...[Container(width: 6, height: 6, decoration: BoxDecoration(color: dotColor, shape: BoxShape.circle)), const SizedBox(width: 5)],
        Text(text, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: textColor)),
      ]));
  }

  void _showUserFormDialog(UserEntity? user) {
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheetState) => Container(
        ...
        child: Form(key: formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Flexible(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 0, 20, 20), child: Column(children: [
            _formField('Username', usernameCtrl, required: true), const SizedBox(height: 12),
            _formField('Email', emailCtrl, required: true, isEmail: true), const SizedBox(height: 12),
            _formField('Password', passwordCtrl, required: !isEdit, obscure: true), const SizedBox(height: 12),
            _formField('Konfirmasi Password', confirmCtrl, required: !isEdit, obscure: true, matchCtrl: passwordCtrl), const SizedBox(height: 12),
            Row(children: [Expanded(child: _formField('Nama Depan', firstNameCtrl)), const SizedBox(width: 10), Expanded(child: _formField('Nama Belakang', lastNameCtrl))]), const SizedBox(height: 12),
            Row(children: [Expanded(child: _formField('Telepon', phoneCtrl)), const SizedBox(width: 10), Expanded(child: _formField('Jabatan', jobTitleCtrl))]), const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _dropdownField('Role', selectedRole, ['admin', 'user'], (v) { setSheetState(() => selectedRole = v!); })),
              const SizedBox(width: 10),
              Expanded(child: _dropdownField('Status', selectedStatus, ['active', 'inactive'], (v) { setSheetState(() => selectedStatus = v!); })),
            ...
            Row(children: [
              ...
              Expanded(child: ElevatedButton(onPressed: () {
                ...
                final body = <String, dynamic>{'username': usernameCtrl.text.trim(), 'email': emailCtrl.text.trim(), 'role': selectedRole, 'account_status': selectedStatus, 'first_name': firstNameCtrl.text.trim(), 'last_name': lastNameCtrl.text.trim(), 'phone': phoneCtrl.text.trim(), 'job_title': jobTitleCtrl.text.trim()};
                if (passwordCtrl.text.isNotEmpty) { body['password'] = passwordCtrl.text; body['password_confirmation'] = confirmCtrl.text; }
                final bloc = context.read<UsersBloc>();
                if (isEdit) { bloc.add(UserUpdate(token: widget.token, id: user.id, body: body)); } else { bloc.add(UserCreate(token: widget.token, body: body)); }
                Navigator.pop(ctx);
  ...
  String _getInitials(UserEntity user) {
    if (user.firstName != null && user.lastName != null && user.firstName!.isNotEmpty && user.lastName!.isNotEmpty) return '${user.firstName![0]}${user.lastName![0]}'.toUpperCase();
    return user.username.isNotEmpty ? user.username[0].toUpperCase() : '?';
  }

  String _getDisplayName(UserEntity user) {
    if (user.firstName != null && user.firstName!.isNotEmpty) return '${user.firstName} ${user.lastName ?? ''}'.trim();
