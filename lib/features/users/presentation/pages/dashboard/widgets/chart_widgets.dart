...
class MiniLineChart extends StatelessWidget {
  ...
  final Color color;
  const MiniLineChart({super.key, required this.data, this.color = AppColors.primary});
  @override
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) => CustomPaint(painter: _LineChartPainter(data: data, color: color), size: Size(constraints.maxWidth, constraints.maxHeight)));
  }
}
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final minVal = data.reduce(min); final maxVal = data.reduce(max);
    ...
    final areaPath = Path(); areaPath.moveTo(points.first.dx, size.height);
    for (int i = 0; i < points.length; i++) {
      if (i == 0) { areaPath.lineTo(points[i].dx, points[i].dy); } else {
        final prev = points[i - 1]; final curr = points[i];
        final cp1 = Offset(prev.dx + (curr.dx - prev.dx) / 2, prev.dy); final cp2 = Offset(prev.dx + (curr.dx - prev.dx) / 2, curr.dy);
    ...
    final linePath = Path();
    for (int i = 0; i < points.length; i++) {
      if (i == 0) { linePath.moveTo(points[i].dx, points[i].dy); } else {
        final prev = points[i - 1]; final curr = points[i];
        final cp1 = Offset(prev.dx + (curr.dx - prev.dx) / 2, prev.dy); final cp2 = Offset(prev.dx + (curr.dx - prev.dx) / 2, curr.dy);
...
class DonutChart extends StatelessWidget {
  final double activeRate; final int activeCount; final int inactiveCount; final int total;
  const DonutChart({super.key, required this.activeRate, required this.activeCount, required this.inactiveCount, required this.total});
  @override
  Widget build(BuildContext context) {
    return Row(children: [
...
class VerticalBarChart extends StatelessWidget {
  ...
  const VerticalBarChart({super.key, required this.data, required this.labels});
  @override
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) => CustomPaint(painter: _BarChartPainter(data: data, labels: labels), size: Size(constraints.maxWidth, constraints.maxHeight)));
  }
}
...
class _BarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (data.isEmpty || size.width <= 0 || size.height <= 0) return;
    final maxVal = data.reduce(max); final barWidth = (size.width / data.length) * 0.55;
    final gap = (size.width / data.length) * 0.45; final chartHeight = size.height - 20;
    for (int i = 0; i < data.length; i++) {
